...
- **执行 AI 里程碑 plan**（M0/M2/M4/M6–M12）时**必做**，与 [implement-plan](../implement-plan/SKILL.md) 阶段 B 同步
- AI 知识点尚未写入 `docs/knowledge/` 时，实现该里程碑时必须新增并更新索引
...
| `function-calling` | Function Calling | M2 | `agent/agent.py`, `agent/skills/` | done |
| `langgraph` | LangGraph 状态图 Agent | M4 | `agent/graph.py`, `agent/agent.py` | 待提取 |
...
1. 定 plan 时：AI 里程碑 frontmatter `skills` 必含 `extract-ai-knowledge`；正文写「知识点文档」章节（slug、要点、验收）
2. 实现代码时：与业务改动同批撰写 `docs/knowledge/{slug}.md`，更新 README 索引
3. 阶段 C 验收：与 implement-plan「AI 里程碑 knowledge 验收」命令一致
