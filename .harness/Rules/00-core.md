...
1. **能复用优先复用** — 现有函数、抽象、配置流程能满足需求时，直接复用，不重复造轮子
2. **能改个名字优先改名字** — 仅需对齐命名或语义时，改名即可，不重写逻辑、不搬目录
3. **能增加参数优先加参数** — 现有接口差一步就能用时，用可选参数扩展行为，不新增平行函数或重复入口（如 `ensure_*` / `get_or_create_*`）
...
- 遵循上方「第一条原则」的优先级
- 优先复用现有函数与抽象，不重复造轮子
- **尽量采用新增的方式，不乱改现有代码**；现有方法能满足需求时，优先扩展参数而非新增平行函数
- **改现有接口前先查已有方法**：若可通过可选参数扩展行为，用参数扩展；不要为同类逻辑再抽 `ensure_*` / `get_or_create_*` 等重复入口
- **改现有接口前先查已有方法**；不要为同类逻辑再抽重复入口
