...
class CategoryService:
    async def create_category(
        self, db: AsyncSession, body: CategoryCreateRequest
    ) -> CategoryCreateResponse:
    ...
    async def get_category(self, db: AsyncSession, category_id: int) -> CategoryGetResponse | None:
    ...
    async def delete_category(self, db: AsyncSession, category_id: int) -> bool:
        existing = await category_crud.get(db, id=category_id)
        if existing is None:
            return False
